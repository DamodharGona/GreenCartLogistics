...
async function main() {
  ...
  try {
    ...
    // Import drivers
    const driversData = fs.readFileSync(
      path.join(__dirname, "../drivers.csv"),
      "utf-8"
    );
    ...
    const routesData = fs.readFileSync(
      path.join(__dirname, "../routes.csv"),
      "utf-8"
    );
    ...
    const ordersData = fs.readFileSync(
      path.join(__dirname, "../orders.csv"),
      "utf-8"
    );
    ...
    console.log(
      `📊 Created ${drivers.length} drivers, ${routes.length} routes, ${orders.length} orders`
    );
  } catch (error) {
